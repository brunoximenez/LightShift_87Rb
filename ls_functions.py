...
def get_dipolematrix_hfs(atom, n1, l1, j1, f1, mf1, n2, l2, j2, f2, mf2, q):
    ...
    elem = atom.getDipoleMatrixElementHFS(
        n1, l1, j1, f1, mf1, n2, l2, j2, f2, mf2, q)
    return elem ** 2
...
def s_states(atom, f, mf, q, P, w0):
    ...
    light_shift_transition = sum_dip_square * \
        get_E_square(P, w0) / (4 * h * det_12)
    # In MHz
...
def get_ls_mfs_5P3_2(atom, f, q, P, w0):
    mf_matrix = np.array([])
    ls_matrix = np.array([])
    title = r'$\omega_{0} = $' + str(w0 * 1e6) + \
        r'$\mu$' + 'm' + ', P = ' + str(P * 1e3) + ' mW'
...
def get_plot_transitions_OP_pushout(atom, w0):
    ...
    for power in p:
        ...
        shift = get_ls_transition5S_5P32(
            atom, f_ground, mf_ground, f_excited, mf_excited, q, power, w0)
...
def plot_pushout_w_ls_zeeman(atom, B, laser_power, w0=1.1e-6):
    q = 0  # trap lasers


    shift_pushout = np.array([])
    # ls_shift_OP = np.array([])
    plt.figure('Power light shift')
    mf_matrix = np.array([])

    # Pushout transition
    for mf_ground in range(-2, 3, 1):
        # Pushout transition
        f_ground = 2
        ...
        shift = get_ls_transition5S_5P32(atom, f_ground, mf_ground, f_excited, mf_excited, q, laser_power, w0)
        shift += get_zeeman(B, mf_ground, mf_excited)
        shift_pushout = np.append(shift_pushout, shift)
        mf_matrix = np.append(mf_matrix, mf_ground)


    plt.plot(mf_matrix, shift_pushout, '_', markersize=30, label='Pushout transition')
    # plt.plot(p*1e3, ls_shift_OP, label='Optical pumping transition')
    # plt.xlabel('Laser power (mW)', fontsize=fontsize)
    plt.ylabel('Shift on transitions (MHz)', fontsize=fontsize)
    plt.xlabel(r'$m_{f_{ground}}$', fontsize=fontsize)
    # plt.xlim((0, 5))
    # plt.ylim((0, 40))
    plt.legend(fontsize=fontsize)
    plt.grid(linewidth=.2)
...
def plot_op_w_ls_zeeman(atom, B, laser_power, w0=1.1e-6):
    q = 0  # trap lasers
    ...
    plt.plot(mf_matrix, shift_pushout, '_', markersize=30, label='Optical pumping transition')
    # plt.plot(p*1e3, ls_shift_OP, label='Optical pumping transition')
    # plt.xlabel('Laser power (mW)', fontsize=fontsize)
    plt.ylabel('Shift on transitions (MHz)', fontsize=fontsize)
    plt.xlabel(r'$m_{f_{ground}}$', fontsize=fontsize)
...
def get_ls_OP_pushout(laser_power, q=0):
    ...
    f_excited = 3
    mf_excited = mf_ground + 1
    shift_pushout = get_ls_transition5S_5P32(
        atom, f_ground, mf_ground, f_excited, mf_excited, q, laser_power, w0)

    # OP transition
    f_excited = 2
    mf_ground = 0
    mf_excited = mf_ground + 1
    shift_OP = get_ls_transition5S_5P32(
        atom, f_ground, mf_ground, f_excited, mf_excited, q, laser_power, w0)

    return shift_OP, shift_pushout


def get_zeeman(B, mf_ground, mf_excited):
    '''
    Returns the Zeeman shift in this
    transition given a B field in Gauss
    B field should be < 100 G
    Returns the shift in MHz
    '''
    zeeman_shift_ground = 0.7
    zeeman_shift_excited = 0.93

    shift = - zeeman_shift_ground * B * mf_ground
    shift += zeeman_shift_excited * B * mf_excited

    return shift


def get_zeeman_pushout(B):
    '''
    Returns the Zeeman shift in this
    transition given a B field in Gauss
    B field should be < 100 G
